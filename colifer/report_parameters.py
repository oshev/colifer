...
class ReportParameters:
    ...
    def __init__(self, config_entries, report_type_name: str, year: int, period_num: int):
        ...
        if report_type_name.lower() not in report_type_values:
            raise ValueError(
                "Non-existent report type {}, supported types: {}".format(report_type_name,
                                                                          report_type_values))
        else:
        ...
        if report_type == ReportType.YEARLY:
            ...
            self.period_value = str(self.period_number)
            self.period_start, self.period_end = self._get_quarter_start_end(self.year,
                                                                             self.period_number)
            ...
            self.period_start, self.period_end = self._get_month_start_end(self.year,
                                                                           self.period_number)
        elif report_type == ReportType.WEEKLY:
            self._abort_if_period_invalid(period_num, 1, 53, "week")
            self.period_value = str(self.period_number)
            self.period_type = "Week"
            self.period_start, self.period_end = self._get_week_start_end(self.year,
                                                                          self.period_number)

        self.period_start_str = self.period_start.strftime("%d.%m")
        self.period_end_str = self.period_end.strftime("%d.%m")
        self.report_title = self.set_variables(
            Config.get_section_param(config_entries, 'report_title'))

        self.pic_dir = self.set_variables(Config.get_section_param(config_entries, 'pic_dir'))
        # Config.make_dir(self.pic_dir)

    @staticmethod
    def _abort_if_period_invalid(period_number: int, min_value: int, max_value: int,
                                 period_name: str):
        if period_number > max_value or period_number < min_value:
            raise ValueError(
                "Wrong {} number, accepted values {}-{}.".format(period_name, min_value,
                                                                 max_value))

    @staticmethod
    def _get_year_start_end(year: int) -> (datetime.datetime, datetime.datetime):
        datetime_start = datetime.datetime.combine(datetime.date(year, 1, 1),
                                                   datetime.datetime.min.time())
        datetime_end = datetime.datetime.combine(datetime.date(year, 12, 31),
    ...
    def _get_quarter_start_end(year: int, quarter: int) -> (datetime.datetime, datetime.datetime):
        ...
        datetime_start = datetime.datetime.combine(date_of_first_day_of_quarter,
                                                   datetime.datetime.min.time())
        datetime_end = datetime.datetime.combine(date_of_last_day_of_quarter,
                                                 datetime.datetime.max.time())
        return datetime_start, datetime_end
    ...
    def _get_month_start_end(year: int, month: int) -> (datetime.datetime, datetime.datetime):
        ...
        datetime_start = datetime.datetime.combine(datetime.date(year, month, 1),
                                                   datetime.datetime.min.time())
    ...
    def _get_week_start_end(year: int, week: int) -> (datetime.datetime, datetime.datetime):
        datetime_start = datetime.datetime.strptime(f"{year}-W{week - 1}" + '-1', "%Y-W%W-%w")
        datetime_end = datetime.datetime.combine(
            datetime_start.date() + datetime.timedelta(days=6), datetime.datetime.max.time()
        )
        return datetime_start, datetime_end
